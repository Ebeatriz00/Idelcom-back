...
namespace Application.Validators.ConceptType
{
    public class ConceptTypeUpdateValidator : AbstractValidator<ConceptTypeUpdateDto>
    {
        public ConceptTypeUpdateValidator()
        {
            RuleFor(x => x.ConceptTypeId)
                .GreaterThan(0).WithMessage("ID del tipo de concepto inválido.")
                .WithErrorCode(ErrorCodes.Conflict);

            RuleFor(x => x.Description)
                .Cascade(CascadeMode.Stop)
                .NotEmpty().WithMessage("La descripción del tipo de concepto es obligatoria.")
