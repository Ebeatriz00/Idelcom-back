...
namespace Application.Validators.Concepts
{
    public class ConceptsUpdateValidator : AbstractValidator<ConceptsUpdateDto>
    {
        public ConceptsUpdateValidator()
        {
            RuleFor(x => x.ConceptsId)
                .GreaterThan(0).WithMessage("ID de concepto inválido.")
            ...
            RuleFor(x => x.ConceptGroupsId)
                .GreaterThan(0).WithMessage("El grupo de concepto es obligatorio.")
                .WithErrorCode(ErrorCodes.ValidationCharacterNegative);
