...
namespace Application.Validators.Products
{
    ...
    public class ProductsCreateValidator : AbstractValidator<ProductsCreateDto>
    {
        public ProductsCreateValidator()
        {
            ...
            RuleFor(x => x.Files)
                .NotNull().WithMessage("Los archivos son obligatorios.")
                .WithErrorCode(ErrorCodes.ValidationEmpty);

            RuleForEach(x => x.Files).ChildRules(file =>
            {
                file.RuleFor(x => x.FileTitle)
                    .MaximumLength(350).WithMessage("El nombre del archivo no debe exceder los 350 caracteres.")
                    .WithErrorCode(ErrorCodes.ValidationLength);

                file.RuleFor(x => x.FileUrl)
                    .MaximumLength(350).WithMessage("La ruta del archivo no debe exceder los 350 caracteres.")
                    .WithErrorCode(ErrorCodes.ValidationLength);

                file.RuleFor(x => x.RelativePath)
                    .MaximumLength(350).WithMessage("La ruta relativa del archivo no debe exceder los 350 caracteres.")
                    .WithErrorCode(ErrorCodes.ValidationLength);
            });
