...
{
    public class CreatePurchaseOrderFromInvoice(
        IPurchaseOrderRepository repository,
        IValidator<PurchaseOrderCreateFromInvoiceDto> validator,
        ...
        IMapper mapper)
    {
        ...
        private readonly IValidator<PurchaseOrderCreateFromInvoiceDto> _validator = validator;
        private readonly ISqlConnectionFactory _sqlConnectionFactory = sqlConnectionFactory;
        private readonly IAuditService _auditService = auditService;
        private readonly IAuditLogFactory _auditLogFactory = auditLogFactory;
        private readonly IMapper _mapper = mapper;

        public async Task<BaseResponseId> ExecuteAsync(PurchaseOrderCreateFromInvoiceDto dto, long userId, long businessId)
        {
            ...
            try
            {
                var command = _mapper.Map<PurchaseOrderCreateFromInvoiceCommand>(dto);
                command.BusinessId = businessId;
                command.UserId = userId;

                var result = await _repository.CreateFromInvoiceAsync(command, transaction);
                ...
                if (result.Id is > 0)
                {
                    ...
                    if (created?.Header is not null)
                    {
                        ...
                        var relation = await _repository.GetPurchaseOrderInvoiceAsync(
                            businessId,
                            result.Id.Value,
                        ...
                        if (relation is not null)
                        {
                            ...
                            await _auditService.RegisterCreateAsync(
                                _mapper.Map<PurchaseOrderInvoice>(relation),
                                relationAuditLog,
                                transaction);
                        }
                    }
