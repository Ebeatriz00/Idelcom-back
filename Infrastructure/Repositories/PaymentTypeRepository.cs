...
namespace Infrastructure.Repositories
{
    public class PaymentTypeRepository : IPaymentTypeRepository
    {
        ...
        public PaymentTypeRepository(ISqlConnectionFactory connectionFactory)
        {
        ...
        public async Task<bool> ExistsAsync(string code, string description, long businessId, long? excludeId = null)
        {
            try
            {
                ...
                string query = "SELECT COUNT(*) FROM PAYMENT_TYPE WHERE DESCRIPTION = @DESC AND CODE = @CODE AND BUSINESS_ID = @BID";

                if (excludeId.HasValue)
                    query += " AND PAYMENT_TYPE_ID <> @ID";
        ...
        public async Task AddAsync(PaymentType entity)
        {
            ...
            {
                using var connection = _connectionFactory.CreateConnection();
                using var cmd = new SqlCommand("SP_WS_REGISTER_PAYMENT_TYPE", connection)
                {
        ...
        public async Task<PagedResult<PaymentType>> GetAllAsync(long businessId, string? search, int page, int pageSize, long? usersBy)
        {
            try
            {
                ...
                using var cmd = new SqlCommand("SP_WS_LIST_PAYMENT_TYPE", cn)
                {
                    CommandType = CommandType.StoredProcedure,
                    CommandTimeout = 15
                };
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    {
                        PaymentTypeId = reader.GetInt64(0),
                        Code = reader.GetString(1),
                        Description = reader.GetString(2),
                        Status = reader.GetString(3),
                        PaymentCount = reader.GetInt32(4),
                ...
                return new PagedResult<PaymentType>
                {
                    Items = list,
                    Page = page,
                    PageSize = pageSize,
        ...
        public async Task<PagedSelect<OptionItem>> GetPaymentTypeForSelectAsync(long businessId, string? search, int page, int pageSize)
        {
            var items = new List<OptionItem>();
            using var cn = _connectionFactory.CreateConnection();
            await cn.OpenAsync();

            using var cmd = new SqlCommand("SP_WS_PAYMENT_TYPE_SELECT", cn) { CommandType = CommandType.StoredProcedure, CommandTimeout = 15 };
            cmd.Parameters.AddWithValue("@BUSINESS_ID", businessId);
            cmd.Parameters.AddWithValue("@SEARCH", (object?)(string.IsNullOrWhiteSpace(search) ? null : search.Trim()) ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@PAGE", page);
            cmd.Parameters.AddWithValue("@PAGE_SIZE", pageSize);
        ...
        public async Task<PaymentType> GetByIdAsync(long paymentTypeId)
        {
            try
            {
                using var cn = _connectionFactory.CreateConnection();
                ...
                using var cmd = new SqlCommand("SP_WS_PAYMENT_TYPE_BY_ID", cn)
                {
                    CommandType = CommandType.StoredProcedure,
                    CommandTimeout = 15
                };
        ...
        public async Task<bool> UpdateAsync(PaymentType entity)
        {
            try
            {
                ...
                using var cmd = new SqlCommand("SP_WS_UPDATE_PAYMENT_TYPE", cn)
                {
                    CommandType = CommandType.StoredProcedure,
                    CommandTimeout = 15
                };
        ...
        public async Task<bool> PatchStatusAsync(long paymentTypeId, string status, int UsersBy, long businessId)
        {
            try
            {
                using var cn = _connectionFactory.CreateConnection();
                ...
                using var cmd = new SqlCommand("SP_WS_UPDATE_PAYMENT_TYPE_ACTIVE", cn)
                {
                    CommandType = CommandType.StoredProcedure,
                    CommandTimeout = 15
                };
