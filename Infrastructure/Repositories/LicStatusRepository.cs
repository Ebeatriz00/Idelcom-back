...
{
    public class LicStatusRepository : ILicStatusRepository
    {
        private readonly ISqlConnectionFactory _connectionFactory;

        public LicStatusRepository(ISqlConnectionFactory connectionFactory)
        {
    ...
    public async Task<PagedSelect<OptionItem>> GetForSelectAsync(long businessId, string? search, int page, int pageSize)
        {
            ...
            using var cmd = new SqlCommand("SP_WS_LIC_STATUS_SELECT", cn) { CommandType = CommandType.StoredProcedure, CommandTimeout = 15 };
