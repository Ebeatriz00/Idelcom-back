...
{
    public class TypeSuppliersRepository : ITypeSuppliersRepository
    {
        private readonly ISqlConnectionFactory _connectionFactory;

        public TypeSuppliersRepository(ISqlConnectionFactory connectionFactory)
        {
        ...
        public async Task<PagedSelect<OptionItem>> GetForSelectAsync(long businessId, string? search, int page, int pageSize)
        {
            try
            {
                ...
                using var cmd = new SqlCommand("SP_WS_SUPPLIERS_TYPE_SELECT", cn) { CommandType = CommandType.StoredProcedure, CommandTimeout = 15 };
